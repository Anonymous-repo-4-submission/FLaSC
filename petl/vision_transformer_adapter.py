...
def vit_base_patch16_224_mocov3_adapter(pretrained=False, **kwargs):
    
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    # breakpoint()

    # checkpoint_model = torch.load('./pretrained_model/B_16-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.0-sd_0.0.npz')
    checkpoint_model=timm.create_model("vit_base_patch16_224_in21k", pretrained=True, num_classes=0)
    ckpt = torch.load('./pretrained/mocov3-vit-base-300ep.pth', map_location='cpu')['model']
    state_dict = checkpoint_model.state_dict()
    state_dict.update(ckpt)
    msg = checkpoint_model.load_state_dict(state_dict, strict=False)
    print("not ubale load these mocov3 weights", msg)
