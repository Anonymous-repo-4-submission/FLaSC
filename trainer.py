...
def train(args):
    ...
    for seed in seed_list:
        args["seed"] = seed
        args["device"] = device
        ave_acc, avg_acc =_train(args)
        ave_accs.append(ave_acc)
    return ave_accs, avg_acc


def _train(args):

    init_cls = 0 if args ["init_cls"] == args["increment"] else args["init_cls"]
    logs_name = "logs/{}_our_approach_21k_1shot/{}/{}/{}".format(args["model_name"],args["dataset"], init_cls, args['increment'])
    if not os.path.exists(logs_name):
        os.makedirs(logs_name)
    logfilename = "logs/{}_our_approach_21k_1shot/{}/{}/{}/{}_{}_{}_{}_shot_{}_class_sample_seed_{}".format(
        args["model_name"],
        args["dataset"],
        init_cls,
        args["increment"],
    ...
    for task in range(num_tasks):
        ...
        acc_total,acc_grouped,predicted_classes,true_classes,features = model.eval_task()
        ...
        logging.info("Ave Inc Acc curve: {}".format(np.mean(acc_curve["ave_acc"])))
        logging.info("Top1 curve: {}".format(acc_curve["top1_total"]))
        
    logging.info('Finishing run')
    logging.info('')
    return acc_curve["ave_acc"], np.mean(acc_curve["ave_acc"])
